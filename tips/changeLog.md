...
- 更新时间：2026.03.21
- Modbus: 添加读取线圈和离散输入字节数据的方法
- MC: 修复软元件代码错误问题
- NET: 修复短连接失败后必须重新new，无法重新connect的问题
- NET：read方法的异常修改为SocketException，这样能捕获IO异常并设置socketError为true，之前是捕获不到

## v1.5.5

- 更新时间：2025.10.10
- MC: 修复SM和SD软元件的访问错误
- MC: 修复MC协议地址解析，主要针对16进制地址解析问题，例如XFF这种
- MC: 测试FX5U PLC所有软元件的读写
- MC：验证R系列PLC访问ZR软元件
- S7：修复300/400 PLC的T区和C区读取
- S7: 添加访问报错时输出对应的错误地址
- S7：ErrorCode静态内部类（static 内部类）实现懒加载，避免不必要的资源消耗
- S7：优化数据字节转换代码
- BYTE: 添加处理字节序列化部分，对字符集可编辑
- DOCS: 修改教程
